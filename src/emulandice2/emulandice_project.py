...
def emulandice_project(
    ...
    targyears = np.arange(pyear_start, pyear_end + 0.5, pyear_step)
    if doRebaseSamples:
        lazy_regions = [
            dask.delayed(
                RebaseSamples(
...
def RebaseSamples(ncfile, targyears, baseyear, cyear_start, cyear_end):
    ...
    ds.to_netcdf(
        ncfile,
        encoding={"sea_level_change": {"dtype": "f4", "zlib": True, "complevel": 4}},
    )
...
def TotalSamples(infiles, outfile, chunksize, ice_source):
    ...
    total_out.attrs = {
        ...
        "source": "FACTS: Post-processed total among available contributors: {}".format(
            ",".join(infiles)
        ),
    }
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run the projection stage for the emulandice SLR projection workflow",
    ...
    parser.add_argument(
        "--pyear_end",
        help="Year for which projections end [default=2300]",
        default=2300,
        type=int,
    )
    parser.add_argument(
